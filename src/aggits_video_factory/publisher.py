...
from .config import GITHUB_REMOTE, PUBLIC_BASE_URL
...
def _library_path(workspace: Path) -> Path:
    return workspace / "public" / "library.json"
...
<html lang="en-AU"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#080706"><title>AGGITS Video Jukeboxes</title><style>
...
</style></head><body><main><h1>AGGITS</h1><p>VIDEO DISCOVERY JUKEBOXES</p><section class="grid">{cards}</section></main></body></html>'''
...
class Publisher:
    ...
    def ensure_workspace(self) -> Path:
        ...
        _run([self.git, "config", "user.name", "AGGITS Video Jukebox Factory"], cwd=workspace)
        _run([self.git, "config", "user.email", "video-jukebox-factory@users.noreply.github.com"], cwd=workspace)
        return workspace
    ...
    def publish(self, project: Project) -> tuple[str, str]:
        ...
        _run([self.git, "add", "--", f"public/{project.slug}", "public/library.json", "public/index.html"], cwd=workspace)
    ...
    def unpublish(self, project: Project) -> str:
        ...
        library = [item for item in _load_library(workspace) if str(item.get("slug")) != project.slug]
        _write_library(workspace, library)
        _run([self.git, "add", "-A", "--", f"public/{project.slug}", "public/library.json", "public/index.html"], cwd=workspace)
        staged = _run([self.git, "diff", "--cached", "--name-only"], cwd=workspace)
        if staged:
