...
from .models import BanjoConfig, Project, ProjectType, Video, WhiteLabelConfig, utc_now
...
def assemble_reviewed_project(
    *,
    ...
    elif project_type is ProjectType.WHITE_LABEL:
        if values.channel_master_config is None or white_label_config is None or any(
            config is not None for config in (values.business_config, values.music_config, values.tourism_config)
        ):
            raise ValueError("Validated White Label configuration is required.")
    else:
    ...
    return Project(
        ...
        channel_master_config=(
            values.channel_master_config
            if project_type in {ProjectType.CHANNEL_MASTER, ProjectType.WHITE_LABEL}
            else None
        ),
        white_label_config=white_label_config if project_type is ProjectType.WHITE_LABEL else None,
...
def assemble_white_label_project(**kwargs) -> Project:
    """White Label adapter over the canonical Channel Master workflow."""
    return assemble_reviewed_project(project_type=ProjectType.WHITE_LABEL, **kwargs)
