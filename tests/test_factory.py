...
class FactoryTests(unittest.TestCase):
    ...
    def test_project_roundtrip_and_site_generation(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("font-size:clamp(11.25px,3.375vw,18.75px)", video_css)
            self.assertIn('[data-machine-state="VIDEO_READY"] .lever{pointer-events:auto!important}', video_css)
            self.assertIn('[data-machine-state="READY_TO_PLAY"] .reel-strip>strong', video_css)
            ...
            self.assertIn('transform:scale(1.1)', video_css)
