...
class FactoryTests(unittest.TestCase):
    ...
    def test_project_roundtrip_and_site_generation(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn("document.querySelector('link[rel=\"canonical\"]')?.href || location.href", script)
            self.assertIn("const data = {title: jukeboxTitle, text: jukeboxTitle, url: canonicalUrl}", script)
            self.assertNotIn("url: activeProjectType === 'banjo' ? location.href : (current?.url || location.href)", script)
            ...
            self.assertRegex(page, r'<script type="module" src="assets/video-machine\.js\?v=[0-9a-f]{12}"></script>')
            ...
            social_cards = list(destination.glob("social-card-v2-*.jpg"))
    ...
    def test_legacy_v2_migration_is_deterministic_and_does_not_invent_cta_data(self):
        ...
        self.assertEqual(encoded["business_config"], {"shop_url": None, "primary_cta": None})
