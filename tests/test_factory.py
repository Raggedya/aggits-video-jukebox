...
class FactoryTests(unittest.TestCase):
    ...
    def test_project_roundtrip_and_site_generation(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertIn('[data-machine-state="READY_TO_PLAY"] .aperture-media', video_css)
            self.assertIn('animation:apertureSeat .38s cubic-bezier(.16,.84,.28,1) both', video_css)
            self.assertIn('@keyframes apertureSeat', video_css)
            self.assertIn('.centre-viewing-gate', video_css)
            self.assertIn('.reel-glass', video_css)
            self.assertIn('.destination-plate', video_css)
            ...
            self.assertIn("data-aperture-media", page)
            self.assertIn("data-destination-title", page)
